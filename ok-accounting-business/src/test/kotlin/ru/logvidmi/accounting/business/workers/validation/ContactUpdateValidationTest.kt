...
class ContactUpdateValidationTest {
    ...
    fun `contact request is copied into contact validating request`() = runTest {
        val context = AccContactContext(
            ...
            contactRequest = AccContact(id = AccContactId("1"), name = "Netflix")
        )
    ...
    fun `company name is trimmed`() = runTest {
        ...
        processor.exec(context)
        Assertions.assertEquals("Netflix", context.contactRequestValidating.name)
    }
    ...
    fun `empty company id causes validation error`() = runTest {
        val context = AccContactContext(
            command = AccCommand.UPDATE,
            contactRequest = AccContact(id = AccContactId(""), name = "Netflix")
        )
    ...
    fun `wrong company id format causes validation error`() = runTest {
        ...
        val context = AccContactContext(
            command = AccCommand.UPDATE,
            contactRequest = AccContact(id = AccContactId(wrongId), name = "Netflix")
        )
    ...
    fun `name field is empty causes validation error`() = runTest {
        val context = AccContactContext(
            command = AccCommand.UPDATE,
            contactRequest = AccContact(id = AccContactId("1"), name = "")
        )
    ...
    fun `after validation contact request is copied to contact validated request`() = runTest {
        ...
        val validatedContactRequest = AccContact(id = AccContactId("1"), name = "Netflix")
