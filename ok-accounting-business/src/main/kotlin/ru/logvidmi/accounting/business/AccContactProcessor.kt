...
import ok.logvidmi.accounting.common.AccSettings
...
class AccContactProcessor(
    val settings: AccSettings,
) {

    suspend fun exec(ctx: AccContactContext) = BusinessChain.exec(
        ctx.apply { settings = this@AccContactProcessor.settings }
    )

    companion object {
        private val BusinessChain = rootChain<AccContactContext>() {
            initStatus("Hello")
            worker("Repository initialization") {
                repository = when(workMode) {
                    AccWorkMode.PROD -> settings.prodRepo
                    AccWorkMode.TEST -> settings.testRepo
                    else -> IContactRepository.NONE
                }
            }
            ...
            operation("Create Contact", AccCommand.CREATE) {
                ...
                validation {
                    worker("Copying request into contactRequestValidating") {
                        contactRequestValidating = contactRequest.deepCopy()
                    }
                    worker("Trimming company name") {
                        contactRequestValidating.name = contactRequestValidating.name.trim()
                    }
            ...
            operation("Read Contact", AccCommand.READ) {
                ...
                validation {
                    ...
                    worker("Trimming company id") {
                        contactRequestValidating.id = AccContactId(contactRequestValidating.id.asString().trim())
                    }
            ...
            operation("Search Contacts", AccCommand.SEARCH) {
                ...
                validation {
                    worker("Copying contactFilterRequest into contactFilterRequestValidating") {
                        contactFilterRequestValidating = contactFilterRequest.deepCopy()
                    }
                    worker("Trimming search string") {
                        contactFilterRequestValidating.searchString = contactFilterRequestValidating.searchString.trim()
                    }
