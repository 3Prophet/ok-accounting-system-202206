...
fun ICorChainDsl<AccContactContext>.stubUpdateContactSuccess(title: String) = worker {
    this.title = title
    on { state == AccState.RUNNING && stubCase == AccStubs.SUCCESS }
    handle {
        state = AccState.FINISHING
        contactResponse = AccContact(id = contactRequest.id, name = contactRequest.name) }
}